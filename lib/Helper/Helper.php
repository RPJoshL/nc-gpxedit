...
class Helper {
     ...
     */
    public static function globRecursive($path, $find, $recursive=True) {
        $result = Array();
        $dh = opendir($path);
        while (($file = readdir($dh)) !== false) {
            ...
            if (is_dir($rfile) and $recursive) {
                foreach (self::globRecursive($rfile, $find) as $ret) {
    ...
    */
    public static function getProgramPath($progname){
